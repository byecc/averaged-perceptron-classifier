...
class APTrain:
    ...
    def new_train(self,parameter,train_encodes,dev_encodes,test_encodes):
        step = 1
        for i in range(parameter.ap_iter_num):
            print('第%d轮迭代：' % (i + 1))
            starttime = time.time()
            ...
            while left_bound < max_len:  # batch
                for encode in train_encodes[left_bound:right_bound]:
                    ...
                    if self.get_maxIndex(sum) != self.get_maxIndex(encode.label):
                        for wi in encode.code_list:
                            self.weight_matrix[wi] += encode.label
                            self.sum_weight_matrix[wi] += np.array(encode.label)*step
                        loss += 1
                    else:
                        cor += 1
                    total += 1
                ...
                if right_bound >= max_len:
                    right_bound = max_len - 1
            self.avg_weight_matrix = self.weight_matrix-self.sum_weight_matrix/step
    ...
    def new_2_train(self,parameter,train_encodes,dev_encodes,test_encodes):
        step = deno = parameter.ap_iter_num*len(train_encodes)
        t = time.time()
        for i in range(parameter.ap_iter_num):
            print('第%d轮迭代：' % (i + 1))
            starttime = time.time()
            ...
            while left_bound < max_len:  # batch
                for encode in train_encodes[left_bound:right_bound]:
                    ...
                    if self.get_maxIndex(sum) != self.get_maxIndex(encode.label):
                        for wi in encode.code_list:
                            self.weight_matrix[wi] += encode.label
                            self.sum_weight_matrix[wi] += (step/deno)*np.array(encode.label)
                        loss += 1
                    else:
                        cor += 1
                    total += 1
